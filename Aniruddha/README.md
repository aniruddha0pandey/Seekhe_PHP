...
- To save precious Bandwidth and develp locally. We need to:
    - Install a web server ([Apache](http://httpd.apache.org/), [Python](https://www.python.org/), etc), or as of PHP 5.4.0, the *CLI* *SAPI* provides a [built-in web server](http://php.net/manual/en/features.commandline.webserver.php) furthermore *Python Webserver* sends your PHP files to th browser or doesn't execute instead get downloads (weird) is likely because it is not configured or able to handle PHP files, it even lack MySQL support another solution write your own python server for executing Dynamic Script (PHP) maybe in future ⭐️.
